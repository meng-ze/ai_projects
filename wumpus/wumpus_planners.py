...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        d = 10000000
        for g in self.goals:
            dis = manhattan_distance_with_heading(node.state, g)
            if d > dis:
                d = dis
        return d
    ...
    def actions(self, state):
        ...
        if state[2] == 0:   # north
            fw = (state[0], state[1]+1, state[2])
            tr = (state[0], state[1], 3)
            tl = (state[0], state[1], state[2] + 1)
        elif state[2] == 1: # west
            fw = (state[0]-1, state[1], state[2])
            tr = (state[0], state[1], state[2] - 1)
            tl = (state[0], state[1], state[2] + 1)
        elif state[2] == 2: # south
            fw = (state[0], state[1]-1, state[2])
            tr = (state[0], state[1], state[2] - 1)
            tl = (state[0], state[1], state[2] + 1)
        elif state[2] == 3: # east
            fw = (state[0]+1, state[1], state[2])
            tr = (state[0], state[1], state[2] - 1)
            tl = (state[0], state[1], 0)
        dw = 100000
        dr = 100000
        dl = 100000
        for g in self.goals:
            w = 100000
            if (fw[0], fw[1]) in self.allowed:
                w = manhattan_distance_with_heading(fw, g)
            r = manhattan_distance_with_heading(tr, g)
            l = manhattan_distance_with_heading(tl, g)
            if dr > r:
                dr = r
            if dw > w:
                dw = w
        ...
        if dw <= dr and dw <= dl:
            actions = ['Forward']
        elif dr <= dw and dr <= dl:
            actions = ['TurnRight']
        elif dl <= dw and dl <= dr:
            actions = ['TurnLeft']
        return actions
    ...
    def result(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        newstate = state
        i = proposition_bases_actions.index(action)
        if i == 0:  # Forward
            if state[2] == 0:   # north
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if (state[0], state[1]) in self.goals:
            return True
        return False
...
class PlanShotProblem(search.Problem):
    def __init__(self, initial, goals, allowed):
        ...
        self.goals = goals     # list of goals that can be achieved
        self.allowed = allowed # the states we can move into
        self.shot_at = []       # the location we can shoot to wumpus
        for a in self.allowed:
            for g in self.goals:
                if a[0] == g[0] and a[1] < g[1]:    # need to face to North
                    self.shot_at.append((a[0], a[1], 0))
                elif a[0] == g[0] and a[1] > g[1]:    # need to face to South
                    self.shot_at.append((a[0], a[1], 2))
                elif a[0] > g[0] and a[1] == g[1]:    # need to face to West
                    self.shot_at.append((a[0], a[1], 1))
                elif a[0] < g[0] and a[1] < g[1]:    # need to face to East
                    self.shot_at.append((a[0], a[1], 3))
    ...
    def h(self,node):
        ...
        d = 10000000
        s = node.state
        for g in self.shot_at:
            d1 = manhattan_distance_with_heading(s, g)
            dis = d1
            if d > dis:
                d = dis
        return d
    ...
    def actions(self, state):
        ...
        same_loc = False
        for a in self.shot_at:
            if (state[0], state[1]) == (a[0], a[1]):
                dis = state[2] - a[2]
                if dis == -1:
                    actions = ['TurnLeft']
                else:
