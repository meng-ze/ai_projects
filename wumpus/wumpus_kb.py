...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i, v in enumerate(tvec):
        ...
        if not v:
            pbpf_str += '~'
        pbpf_str += proposition_bases_perceptual_fluents[i] + str(t)
        tvec_str.append(pbpf_str)
    axiom_str = ' & '.join(tvec_str)
    #print axiom_str
    return axiom_str
...
#-------------------------------------------------------------------------------
def axiom_generator_location_check(x, y, xmin, xmax, ymin, ymax):
    return x >= xmin and x <= xmax and y >= ymin and y <= ymax

def axiom_generator_loc_neighbor(x, y):
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    pits = []
    for (xtmp, ytmp) in axiom_generator_loc_neighbor(x,y):
        if axiom_generator_location_check(xtmp,ytmp,xmin,xmax,ymin,ymax):
            pits.append(pit_str(xtmp, ytmp))
    axiom_str += ' | '.join(pits)
    axiom_str += ')'
    #print axiom_str
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for (xtmp, ytmp) in axiom_generator_loc_neighbor(x,y):
        if axiom_generator_location_check(xtmp,ytmp,xmin,xmax,ymin,ymax):
            wumpuses.append(wumpus_str(xtmp, ytmp))
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin, xmax+1):
        for y in range(ymin, ymax+1):
            wumpuses.append(wumpus_str(x,y))
    axiom_str += ' | '.join(wumpuses)
    #print axiom_str
    return axiom_str
...
#            option = '('
            notW = []
            for xtmp in range(xmin, xmax+1):
                for ytmp in range(ymin, ymax+1):
                    if xtmp == x and ytmp ==y:
                        pass
                    else:
                        notW.append('('+option+' | ~'+wumpus_str(xtmp,ytmp) + ')')
#            option += ' & '.join(notW)
#            option += ')'
#            pos.append('('+option+')')
            pos.append('(' + ' & '.join(notW) +')')
#            not_exists += '~' + wumpus_str(x,y)
#    axiom_str += '('
#    axiom_str += ' | '.join(pos) 
    axiom_str += ' & '.join(set(pos)) 
    axiom_str = '(' + axiom_str + ')'
#    axiom_str += ' & (' 
#    axiom_str += ' & '.join(not_exists) 
#    axiom_str += ')'
    #print axiom_str
    return axiom_str
...
#    axiom_str += state_loc_str(xi, yi, t) + ' <=> ('
    ...
    for x in range(xmin, xmax+1):
        for y in range(ymin, ymax+1):
            if y == yi and x == xi:
                locs.append(state_loc_str(x, y, t))
#                pass
            else:
                locs.append('~' + state_loc_str(x, y, t))
    axiom_str += ' & '.join(locs)
...
#    notHeading = []
    ...
    for h in headingList:
        if h.upper() == heading.upper():
            heads.append('Heading' + h + str(t))
        else:
            heads.append('~Heading' + h + str(t))
    axiom_str += ' & '.join(heads)
    #print axiom_str
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    pos = []
    if axiom_generator_location_check(x-1, y, xmin, xmax, ymin, ymax):
        pos.append(state_loc_str(x-1,y,t))
    if axiom_generator_location_check(x+1, y, xmin, xmax, ymin, ymax):
        pos.append(state_loc_str(x+1,y,t))
    if axiom_generator_location_check(x, y-1, xmin, xmax, ymin, ymax):
        pos.append(state_loc_str(x,y-1,t))
    if axiom_generator_location_check(x, y+1, xmin, xmax, ymin, ymax):
        pos.append(state_loc_str(x,y+1,t))
    axiom_str += '(' + ' | '.join(pos) + ')'
    axiom_str += ' & ' + action_forward_str(t)
    axiom_str += ') | '
...
def axiom_generator_only_one_action_axioms(t):
    ...
    # utils.print_not_implemented()
    acts = []
    for i, a1 in enumerate(proposition_bases_actions):
        act_str = a1 + str(t) + ' <=> ('
        other_act = []
        for j, a2 in enumerate(proposition_bases_actions):
            if i != j:
                other_act.append('~'+a2+str(t))
        act_str += ' & '.join(other_act)
        act_str += ')'
        acts.append('('+act_str+')')
#    axiom_str += '('
    axiom_str += ' & '.join(acts)
